...
static int nlm_range(size_t size, size_t offset, size_t length) {
    return byte_span(size, offset, length) && (!length || offset >= NLM_HEADER_SIZE);
}

static int nlm_fixup(uint32_t value, size_t code_size, size_t data_size) {
    size_t offset = value & UINT32_C(0x3fffffff);
    return byte_span(value & UINT32_C(0x40000000) ? code_size : data_size, offset, 4);
}
...
static int nlm_imports(const uint8_t *data, size_t size, size_t start,
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (!byte_span(size, refs_offset, 4)) return nlm_bad(info);
        size_t refs = read_le32(data + refs_offset);
        refs_offset += 4;
        ...
        for (size_t j = 0; j < refs; ++j)
            if (!nlm_fixup(read_le32(data + refs_offset + j * 4), code_size, data_size))
                return nlm_bad(info);
        size_t end = refs_offset + refs * 4;
        executableRow *row = exe_add(info, EXE_IMPORT, cursor, end - cursor,
                                     "Import: %s [global; %zu references]", name, refs);
        if (!row) return 0;
        row->name_offset = chars;
        row->name_length = length;
        row->editable = 1;
...
static int nlm_symbols(const uint8_t *data, size_t size, size_t start,
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        int code;
        if (debug) {
            if (!byte_span(size, cursor, 6) || data[cursor] > 1) return nlm_bad(info);
            code = data[cursor];
            address = read_le32(data + cursor + 1);
            cursor += 5;
            if (!nlm_name(data, size, cursor, name, &chars, &length))
                return nlm_bad(info);
            cursor = chars + length;
            ...
            if (!nlm_name(data, size, cursor, name, &chars, &length))
                return nlm_bad(info);
            cursor = chars + length;
            if (!byte_span(size, cursor, 4)) return nlm_bad(info);
            address = read_le32(data + cursor);
...
int exe_parse_nlm(const uint8_t *data, size_t size, executableInfo *info) {
    ...
    if (memcmp(data, NLM_SIGNATURE, 24))
    ...
    if (read_le32(data + 24) != 4)
        return exe_error(info, EXE_UNSUPPORTED, "Only i386 NLM v4 import tables are supported");
    size_t module_length = data[28];
    size_t header_end;
    if (!nlm_header_end(data, size, &header_end)) return nlm_bad(info);
    size_t code_offset = read_le32(data + 42), code_size = read_le32(data + 46);
    size_t data_offset = read_le32(data + 50), data_size = read_le32(data + 54);
    size_t bss_size = read_le32(data + 58);
    size_t custom_offset = read_le32(data + 62), custom_size = read_le32(data + 66);
    ...
    for (size_t field = 110; field <= 118; field += 4) {
        size_t address = read_le32(data + field);
        if (address && address >= code_size) return nlm_bad(info);
    }
    if (!exe_add(info, EXE_HEADER, 0, header_end, "NLM v4 i386: %.*s | entry code+0x%X",
                 (int)module_length, (const char *)data + 29, read_le32(data + 110))) return 0;
    if (code_size && !exe_add(info, EXE_REGION, code_offset, code_size,
                             "Code image: %zu bytes; entry file 0x%zX",
                             code_size, code_offset + read_le32(data + 110))) return 0;
    ...
    if (!nlm_names(data, size, read_le32(data + 70), read_le32(data + 74), info, ranges + 3) ||
        !nlm_imports(data, size, read_le32(data + 86), read_le32(data + 90),
                     code_size, data_size, info, ranges + 4) ||
        !nlm_symbols(data, size, read_le32(data + 94), read_le32(data + 98),
                     code_size, data_size, bss_size, 0, info, ranges + 5) ||
        !nlm_symbols(data, size, read_le32(data + 102), read_le32(data + 106),
                     code_size, data_size, bss_size, 1, info, ranges + 6)) return 0;
    size_t fixups = read_le32(data + 82), fixup_offset = read_le32(data + 78);
    ...
    for (size_t i = 0; i < fixups; ++i)
        if (!nlm_fixup(read_le32(data + fixup_offset + i * 4), code_size, data_size))
            return nlm_bad(info);
    for (size_t i = 0; i < 8; ++i) {
        if (ranges[i].length && ranges[i].start < header_end) return nlm_bad(info);
        for (size_t j = i + 1; j < 8; ++j)
            if (byte_overlap(ranges[i].start, ranges[i].length,
                             ranges[j].start, ranges[j].length)) return nlm_bad(info);
    }

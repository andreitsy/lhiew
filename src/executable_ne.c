...
static int ne_module_name(const uint8_t *data, size_t size, size_t modules,
    ...
    return ne_name(data, size, pool, end, starts,
                   exe_u16(data + modules + (index - 1) * 2),
                   name, offset, length);
}
...
static int ne_segments_disjoint(const uint8_t *data, size_t size, size_t table,
    ...
    for (size_t i = 0; i < segments; ++i) {
        ...
        size_t sector = exe_u16(record), length = exe_u16(record + 2);
        unsigned flags = exe_u16(record + 4);
        if (!sector) {
        ...
        if (offset < metadata_end || !exe_span(size, offset, length)) goto malformed;
        size_t end = offset + length;
        if (flags & 0x100) {
            if (!exe_span(size, end, 2)) goto malformed;
            size_t relocations = exe_u16(data + end);
            if (!exe_span(size, end + 2, relocations * 8)) goto malformed;
            end += 2 + relocations * 8;
        }
...
int exe_parse_ne(const uint8_t *data, size_t size, size_t header, executableInfo *info) {
    ...
    if ((exe_u16(h + 12) & 0x0800) && (h[54] == 2 || h[54] == 4))
        return exe_error(info, EXE_UNSUPPORTED, "Self-loading NE modules are unsupported");

    size_t segments = exe_u16(h + 28);
    size_t modules = exe_u16(h + 30);
    size_t segment_table = exe_u16(h + 34);
    size_t module_table = exe_u16(h + 40);
    size_t names = exe_u16(h + 42);
    size_t entry = exe_u16(h + 4);
    size_t entry_length = exe_u16(h + 6);
    unsigned shift = exe_u16(h + 50);
    size_t remaining = size - header;
    if (shift >= sizeof(size_t) * CHAR_BIT ||
        (segments && (segment_table < 64 || !exe_span(remaining, segment_table, segments * 8))) ||
        (modules && (module_table < 64 || !exe_span(remaining, module_table, modules * 2))) ||
        (entry_length && (entry < 64 || !exe_span(remaining, entry, entry_length))))
    ...
    if (modules && (names < module_table + modules * 2 || entry <= names ||
                    !exe_span(remaining, names, entry - names)))
    ...
    for (size_t at = pool; at < pool_end;) {
        ...
        if (!exe_span(pool_end, at + 1, length))
    ...
    for (size_t i = 0; i < modules; ++i) {
        ...
        snprintf(label, sizeof(label), "Module %zu: %s", i + 1, name);
        executableRow *row = exe_add(info, EXE_MODULE, module_table + i * 2, 2, label);
        if (!row) return 0;
    ...
    for (size_t i = 0; i < segments; ++i) {
        ...
        size_t sector = exe_u16(data + record);
        size_t length = exe_u16(data + record + 2);
        unsigned flags = exe_u16(data + record + 4);
        char label[576];
        if (!sector) {
            if (flags & 0x100)
                return exe_error(info, EXE_MALFORMED, "NE relocation segment has no file data");
            snprintf(label, sizeof(label), "Segment %zu: no file data", i + 1);
            if (!exe_add(info, EXE_REGION, record, 8, label)) return 0;
            continue;
        }
        if (sector > (SIZE_MAX >> shift))
            return exe_error(info, EXE_MALFORMED, "NE segment offset overflows");
        size_t offset = sector << shift;
        if (!length) length = 65536;
        if (offset < metadata_end || !exe_span(size, offset, length))
            return exe_error(info, EXE_MALFORMED, "Invalid NE segment data span");
        snprintf(label, sizeof(label), "Segment %zu: %s%s", i + 1,
                 flags & 1 ? "data" : "code", flags & 8 ? " (iterated)" : "");
        if (!exe_add(info, EXE_REGION, offset, length, label)) return 0;
        if (!(flags & 0x100)) continue;
        size_t relocations = offset + length;
        if (!exe_span(size, relocations, 2))
            return exe_error(info, EXE_MALFORMED, "Truncated NE relocation count");
        size_t count = exe_u16(data + relocations);
        relocations += 2;
        if (!exe_span(size, relocations, count * 8))
            return exe_error(info, EXE_MALFORMED, "Truncated NE relocation table");
        ...
        for (size_t j = 0; j < count; ++j) {
            ...
            size_t name_offset, name_length;
            unsigned module_index = exe_u16(data + relocation + 4);
            ...
            unsigned target = exe_u16(data + relocation + 6);
            if (kind == 2) {
                ...
                snprintf(label, sizeof(label), "%s!%s [segment %zu]", module, name, i + 1);
            } else {
                snprintf(label, sizeof(label), "%s!#%u [segment %zu]", module, target, i + 1);
            }
            executableRow *row = exe_add(info, EXE_IMPORT, relocation, 8, label);
            if (!row) return 0;
            if (kind == 2) {
                row->name_offset = name_offset;
                row->name_length = name_length;
